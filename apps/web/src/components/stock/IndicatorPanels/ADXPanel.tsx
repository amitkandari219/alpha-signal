/**
 * ADX Sub-Chart Panel - Average Directional Index
 */
...
export const ADXPanel: React.FC<ADXPanelProps> = ({
  data,
  chartData,
  height = 80,
  onClose,
  ...
  const mergedData = useMemo(() => {
    return chartData.map((item) => {
      const adxPoint = data.find((d) => d.time === item.rawTime);
      const adxValue = adxPoint?.value || null;
      return {
        ...item,
        adx: adxValue,
        adxColor: adxValue && adxValue > 25 ? '#3FB950' : '#484F58',
      };
    });
    // Gap at start is correct - shows warmup period
  }, [chartData, data]);

  const currentADX = data.length > 0 ? data[data.length - 1].value : 0;
      ...
      return (
        <div className="bg-bg-secondary border-2 border-accent-blue rounded-lg p-2 shadow-2xl">
          <div className="text-xs">
            ...
            <div className="font-data" style={{ color: data.adxColor }}>
              ADX: {data.adx?.toFixed(2) || 'N/A'}
            </div>
          </div>
        </div>
      );
    ...
    <div className={`relative border-t border-border-default ${className}`}>
      <div className="flex items-center justify-between px-3 py-1.5 bg-bg-tertiary">
        <div className="flex items-center gap-2">
          <span className="text-xs font-semibold text-text-primary">ADX (14)</span>
          <span
          ...
          >
            {isTrending ? 'Trending' : 'Range-bound'}
          </span>
        </div>
      ...
      <ResponsiveContainer width="100%" height={height}>
        ...
        >
          ...
          <ReferenceLine
            ...
            stroke="#30363D"
            strokeDasharray="5 5"
            label={{ value: 'Trend threshold', fontSize: 9, fill: '#8B949E' }}
          />
          ...
          <Line
            type="monotone"
            dataKey="adx"
            stroke={isTrending ? '#3FB950' : '#484F58'}
            strokeWidth={1.5}
            dot={false}
            connectNulls
          />
        ...
        <div className="absolute top-2 right-2 pointer-events-none z-50">
          <div className="bg-bg-secondary border-2 border-accent-blue rounded-lg p-2 shadow-2xl">
            <div className="text-xs">
              ...
              <div className="font-data" style={{ color: mergedData[crosshairState.activeIndex].adxColor }}>
                ADX: {mergedData[crosshairState.activeIndex].adx?.toFixed(2) || 'N/A'}
              </div>
            </div>
          </div>
        </div>
